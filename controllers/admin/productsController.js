    ...
    try {
        ...
        const productVariants = products.map((product) => {
            ...
            const lowestVariant = product.variants
                .filter((v) => v)
                .reduce((min, v) =>
                    Number(v.volume) < Number(min.volume) ? v : min
                );
        ...
        const variants = Array.isArray(volume)
            ? volume.map((vol, i) => {
                  ...
                  if (!vol) {
                      req.flash(
                          "error",
                          `Variant ${i + 1}: Volume is required`
                      );
                  ...
                  return {
                      ...
                      discount: dp,
                      discountPercentage:
                          bp > 0 ? getDiscountPercent(bp, dp) : 0,
            ...
            : [
                  {
                      ...
                      discountPercentage:
                          basePrice > 0
                              ? getDiscountPercent(basePrice, discountPrice)
                              : 0,
    ...
    try {
        ...
        for (let i = 0; i < volumes.length; i++) {
            ...
            variants.push({
                ...
                basePrice: Number(
                    basePrices[i] || product.variants[i]?.basePrice || 0
                ),
                discount: Number(
